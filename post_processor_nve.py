#!/usr/bin/env python3
"""
Post processor for NVE simulations - Python version
Processes LAMMPS output data for NVE ensemble simulations
Supports both local files and remote SFTP access
Supports both CSV and TXT input files

Usage Examples:
  Local TXT file:
    python post_processor_nve.py data.txt output.csv
  
  Local CSV file:
    python post_processor_nve.py data.csv output.csv
  
  Remote SFTP file:
    python post_processor_nve.py user@server:/path/data.txt output.csv
    python post_processor_nve.py user@server:/path/data.csv output.csv
...
if len(sys.argv) != 3:
    print("Usage: python post_processor_nve.py <input_file_or_url> <output_file>")
    print("Examples:")
    print("  python post_processor_nve.py data.txt output.csv")
    print("  python post_processor_nve.py user@server:/path/data.txt output.csv")
    sys.exit(1)
...
for i in range(1, 4):  # 1, 2, 3
    ...
    outputdata[strain_col_name] = np.concatenate([[0], strain_increments])

for i in range(4, 7):  # 4, 5, 6
    col_name = f"l_{i}"
    strain_col_name = f"de_{i}"
    col_vals = outputdata[col_name].to_numpy()
    outputdata[strain_col_name] = np.concatenate([[0], np.diff(col_vals)])
...
ds_1 = outputdata['ds_1'].to_numpy()
ds_2 = outputdata['ds_2'].to_numpy()
ds_3 = outputdata['ds_3'].to_numpy()
ds_4 = outputdata['ds_4'].to_numpy()
ds_5 = outputdata['ds_5'].to_numpy()
ds_6 = outputdata['ds_6'].to_numpy()

outputdata['dee_1'] = S11 * ds_1 + S12 * (ds_2 + ds_3)
outputdata['dee_2'] = S11 * ds_2 + S12 * (ds_1 + ds_3)
outputdata['dee_3'] = S11 * ds_3 + S12 * (ds_1 + ds_2)
outputdata['dee_4'] = S44 * ds_4
outputdata['dee_5'] = S44 * ds_5
outputdata['dee_6'] = S44 * ds_6
...
for i in range(1, 7):
    ...
    dee_col = f"dee_{i}"
    dep_col = f"dep_{i}"
    outputdata[dep_col] = outputdata[de_col].to_numpy() - outputdata[dee_col].to_numpy()
...
# Heat calculation
outputdata['Delta_Q'] = rho * (1000 * 903) * (t - t[0])

# Energy derivatives
outputdata['Delta_Etot'] = outputdata['Delta_U'].to_numpy()
outputdata['Delta_Ttally'] = outputdata['Delta_T'].to_numpy()
...
outputdata['Beta_1_diff'] = 1 - ((dU - dWe - dQ) / (dWp + 1e-25))
outputdata['Beta_1_int'] = 1 - ((Delta_U - We - Delta_Q) / (Wp + 1e-25))

# Linear regression for stress-strain relationship
elastic_data = outputdata[outputdata['e_6'] <= 0.002]

if len(elastic_data) > 1:
    X = elastic_data['e_6'].to_numpy().reshape(-1, 1)
    y = elastic_data['s_6'].to_numpy()
